...
def predict_diabetes(X):
    ...
    individual_results = {
        "Logistic Regression": "Person is Diabetic" if log_reg.predict(X)[0] == 1 else "Person is Not Diabeti",
        "Random Forest": "Person is Diabetic" if rf.predict(X)[0] == 1 else "Person is Not Diabetic",
        "SVM": "Person is Diabetic" if svm.predict(X)[0] == 1 else "Person is Not Diabeti",
        "Naive Bayes": "Person is Diabetic" if nb.predict(X)[0] == 1 else "Person is Not Diabeti",
...
if predict_button:
    ...
    if avg_result >= 50:
        st.success(f"Average Percentage of Being Diabetic: {avg_result:.2f}%")
    else:
        st.error(f"Average Percentage of Being Diabetic: {avg_result:.2f}%")

    # Display individual results for each algorithm
    st.subheader("Result of Indivisual Algorithms")
